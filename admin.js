...
let allResultsCache = []; // New cache for results
...
window.switchView = function(viewId) {
    ...
    const resultsPdfBtn = document.getElementById('btn-results-pdf');
    const resultsExcelBtn = document.getElementById('btn-results-excel');
    const genericPdfBtn = document.getElementById('btn-generic-pdf');
    const genericExcelBtn = document.getElementById('btn-generic-excel');

    // Hide all specific buttons first
    const btns = [squadPdfBtn, squadExcelBtn, matchPdfBtn, matchExcelBtn, resultsPdfBtn, resultsExcelBtn, genericPdfBtn, genericExcelBtn];
    btns.forEach(btn => btn?.classList.add('hidden'));

    if (viewId === 'teams') {
        squadPdfBtn?.classList.remove('hidden');
        squadExcelBtn?.classList.remove('hidden');
    } 
    else if (viewId === 'matches') {
        matchPdfBtn?.classList.remove('hidden');
        matchExcelBtn?.classList.remove('hidden');
    }
    else if (viewId === 'results') {
        resultsPdfBtn?.classList.remove('hidden');
        resultsExcelBtn?.classList.remove('hidden');
    }
    else if (['users', 'registrations'].includes(viewId)) {
        genericPdfBtn?.classList.remove('hidden');
        genericExcelBtn?.classList.remove('hidden');
    }
    ...
    if(viewId === 'results') loadResultsList();
}
...
window.resetMatchFilters = function() {
    ...
    document.getElementById('filter-match-status').value = 'Scheduled';
...
// --- 12. RESULTS VIEW LOGIC ---

window.loadResultsList = async function() {
    const tbody = document.getElementById('results-table-body');
    if(!tbody) return;
    tbody.innerHTML = '<tr><td colspan="6" class="p-8 text-center text-gray-400">Loading results...</td></tr>';

    const results = await fetchAllRecords('performance_results', '*', 'created_at', false);
    allResultsCache = results || [];

    const events = [...new Set(allResultsCache.map(r => r.event_name))].sort();
    const eventSelect = document.getElementById('filter-result-event');
    if(eventSelect && eventSelect.children.length <= 1) {
...
window.filterResults = function() {
    ...
    const eventFilter = document.getElementById('filter-result-event')?.value || '';
    const genderFilter = document.getElementById('filter-result-gender')?.value || '';
    const classFilter = document.getElementById('filter-result-class')?.value || '';

    let filtered = allResultsCache.filter(r => {
        const matchesSearch = r.event_name.toLowerCase().includes(search) || r.student_name.toLowerCase().includes(search);
        const matchesEvent = eventFilter === '' || r.event_name === eventFilter;
        const matchesGender = genderFilter === '' || r.gender === genderFilter;
        const matchesClass = classFilter === '' || r.category === classFilter;
        return matchesSearch && matchesEvent && matchesGender && matchesClass;
    });

    dataCache = filtered.map(r => ({
        "Event": r.event_name,
        "Category": r.category,
        "Gender": r.gender,
        "Rank": r.rank,
...
function renderResults(data) {
    ...
    tbody.innerHTML = data.map(r => {
        const medalColor = r.medal === 'gold' ? 'text-amber-500' : r.medal === 'silver' ? 'text-slate-400' : 'text-orange-600';
        return `
        <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
            <td class="p-4 font-bold text-gray-900">${r.event_name}</td>
            <td class="p-4"><span class="bg-gray-100 px-2 py-1 rounded text-[10px] font-bold uppercase text-gray-500">${r.category}</span></td>
            <td class="p-4 text-sm">${r.gender}</td>
            <td class="p-4 font-mono font-bold">#${r.rank}</td>
            <td class="p-4 text-sm font-semibold">${r.student_name}</td>
            <td class="p-4 text-right">
                <span class="inline-flex items-center gap-1 font-black uppercase text-xs ${medalColor}">
                    <i data-lucide="award" class="w-4 h-4"></i> ${r.medal}
                </span>
            </td>
        </tr>`;
    }).join('');
...
window.resetResultFilters = function() {
    ...
    document.getElementById('filter-result-event').value = '';
    document.getElementById('filter-result-gender').value = '';
    document.getElementById('filter-result-class').value = '';
    filterResults();
}

window.downloadResultsExcel = function() {
    if (!dataCache || dataCache.length === 0) return showToast("No results to export", "error");
    const ws = XLSX.utils.json_to_sheet(dataCache);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Medalists");
    XLSX.writeFile(wb, `Urja_Results_${new Date().toISOString().split('T')[0]}.xlsx`);
    showToast("Results Excel Downloaded", "success");
}

window.downloadResultsPDF = function() {
    if (!dataCache || dataCache.length === 0) return showToast("No results to export", "error");
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    doc.setFontSize(22);
